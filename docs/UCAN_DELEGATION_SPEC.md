...
Customer (space owner)
    ...
    └── Delegates to: ucanlog service DID
            ...
            │
            ├── Capability: space/blob/replicate
            │   └── Resource: did:key:z6Mk... (customer's space)
            │
...
### 2. `space/blob/replicate`
Allows the service to replicate blobs across multiple storage nodes (typically 3). This is **critical** for long-term content availability and discoverability via IPFS gateways. Without replication, blobs may become unreachable after 18-24 hours if the single storage node becomes unavailable.
...
dlg, err := delegation.Delegate(
    ...
    []ucan.Capability[ucan.NoCaveats]{
        ...
        ucan.NewCapability("space/blob/replicate", spaceDID, ucan.NoCaveats{}),
