...
// Append adds an entry to a log and returns the full append receipt.
...
func (s *LogService) Append(ctx context.Context, logID string, data []byte, dlg delegation.Delegation) (*tlog.AppendReceipt, error) {
	index, err := s.tlogManager.AddEntryWithDelegation(ctx, logID, data, dlg)
	if err != nil {
		return nil, err
	}
	if r := s.tlogManager.LastAppendReceipt(logID); r != nil {
		return r, nil
	}
	// Fallback: receipt data not available (e.g. sync fallback path).
	return &tlog.AppendReceipt{Index: index}, nil
}
