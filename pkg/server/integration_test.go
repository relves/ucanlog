...
func TestGetHead_FromSeparateTables(t *testing.T) {
	...
	// Scenario 2: GetHead reads from latest_head_car
	t.Run("ReadsFromLatestHeadCAR", func(t *testing.T) {
		err = store.SetLatestHeadCAR(ctx, logDID, 42, "bafyHead1", []byte{0x01, 0x02})
		require.NoError(t, err)
	...
	// Scenario 3: Updates to latest_head_car are reflected in GetHead
	t.Run("LatestHeadCARUpdatesReflected", func(t *testing.T) {
		err = store.SetLatestHeadCAR(ctx, logDID, 100, "bafyHead2", []byte{0x03})
		require.NoError(t, err)

		indexCID, treeSize, err := store.GetHead(ctx, logDID)
		require.NoError(t, err)
		assert.Equal(t, "bafyHead2", indexCID)
		assert.Equal(t, uint64(100), treeSize)
	})

	// Scenario 4: Simulate optimistic concurrency check with stale head
