...
// encodeCheckpoint base64-encodes the raw checkpoint bytes, returning empty string on nil.
...
func appendHandler(serviceDID string, logService *logSvc.LogService, storeManager interface{}, validator RequestValidator) server.HandlerFunc[capabilities.AppendCaveats, capabilities.AppendSuccess, capabilities.AppendFailure] {
	return func(
		...
		// The receipt contains all data needed for the response — no extra reads required.
		receipt, err := logService.Append(ctx, spaceDID, data, dlg)
		if err != nil {
			return result.Error[capabilities.AppendSuccess](capabilities.NewAppendFailure(
				"AppendFailed",
		...
		return result.Ok[capabilities.AppendSuccess, capabilities.AppendFailure](capabilities.AppendSuccess{
			Index:      int64(receipt.Index),
			TreeSize:   receipt.TreeSize,
			Checkpoint: encodeCheckpoint(receipt.Checkpoint),
			HeadCID:    receipt.HeadCID,
			RevHeadCID: receipt.RevocationsHeadCID,
		}), nil, nil
	}
}
