...
type LogInstance struct {
	...
	lastReceipt *AppendReceipt // populated after each successful append
}
...
type Manager struct {
	...
	clientPool    clientPooler     // Pool of per-log delegated clients
}

// clientPooler is the interface *storacha.ClientPool satisfies.
// Defined as an interface so tests can inject a mock without network calls.
type clientPooler interface {
	GetClient(spaceDID string, dlg delegation.Delegation) (storacha.StorachaClient, error)
}
...
func (m *Manager) CreateLogWithDelegation(ctx context.Context, logID string, spaceDID string, dlg delegation.Delegation) error {
	...
	// Create a delegated client for this log's space
	client, err := m.clientPool.GetClient(spaceDID, dlg)
	if err != nil {
...
// A fresh GuppyClient is created per call, ensuring each write uses its own delegation.
func (m *Manager) AddEntryWithDelegation(ctx context.Context, logID string, data []byte, dlg delegation.Delegation) (uint64, error) {
	...
	// Ensure log is in memory (lazy restore if needed).
	if _, err := m.GetLogInstance(ctx, logID); err != nil {
		return 0, fmt.Errorf("log %s not found: %w", logID, err)
	}

	m.mu.Lock()
	instance := m.logs[logID]
	m.mu.Unlock()

	// Create a fresh per-request client using the log's recorded spaceDID (which
	// may differ from logID — e.g. revocation logs use "spaceDID-revocations" as
	// their logID but the customer's actual space DID for uploads).
	client, err := m.clientPool.GetClient(instance.SpaceDID, dlg)
	if err != nil {
		return 0, fmt.Errorf("failed to get delegated client: %w", err)
	}

	if st, ok := instance.Driver.(*storacha.Storage); ok {
		st.SetClient(client)
	}
	...
	// Stash receipt on instance for AppendReceiptFor.
	m.mu.Lock()
	instance = m.logs[logID]
	m.mu.Unlock()
	var revocationsHeadCID string
	if m.storeManager != nil {
		revLogID := logID + "-revocations"
		store, err := m.storeManager.GetStore(revLogID)
		if err == nil {
			revocationsHeadCID, _, _, _ = store.GetLatestHeadCAR(ctx, revLogID)
		}
	}
	if st, ok := instance.Driver.(*storacha.Storage); ok {
		r := st.LastFlushReceipt()
		instance.lastReceipt = &AppendReceipt{
			Index:              index,
			TreeSize:           r.TreeSize,
			HeadCID:            r.HeadCID,
			Checkpoint:         r.Checkpoint,
			RevocationsHeadCID: revocationsHeadCID,
		}
	}
	return index, nil
}
...
// LastAppendReceipt returns the receipt from the most recent append to logID.
func (m *Manager) LastAppendReceipt(logID string) *AppendReceipt {
	m.mu.RLock()
	defer m.mu.RUnlock()
	if instance, ok := m.logs[logID]; ok {
		return instance.lastReceipt
	}
	return nil
}

// Close shuts down all active log drivers (e.g. upload queue workers).
// Call this on server shutdown.
func (m *Manager) Close() {
	m.mu.RLock()
	defer m.mu.RUnlock()
	for _, instance := range m.logs {
		if st, ok := instance.Driver.(*storacha.Storage); ok {
			st.Close()
		}
	}
...
// RecoverLog re-seeds the state stores for logID (and its revocations log) from
// the IPFS gateway after a total DB wipeout.
//
// headCID is required — it is the last known headCID for the main log.
//
// revocationsHeadCID must be supplied if the revocations log ever had entries
// written to it. Pass an empty string only if you are certain no revocations
// were ever recorded; omitting it when revocations existed leaves the log
// exposed to formerly revoked credentials.
//
// After a successful RecoverLog the normal ModeResume startup path will find
// the restored state for both logs.
func (m *Manager) RecoverLog(ctx context.Context, logID string, headCID string, revocationsHeadCID string) error {
	...
	// Recover the main log first.
	stateStore, err := m.storeManager.GetStore(logID)
	if err != nil {
		return fmt.Errorf("failed to get state store for %s: %w", logID, err)
	}
	if err := storacha.Recover(ctx, storacha.Config{
		SpaceDID:   logID,
		LogDID:     logID,
		StateStore: stateStore,
		...
		return fmt.Errorf("recover main log: %w", err)
	}

	// Recover the revocations log if it had prior entries.
	if revocationsHeadCID != "" {
		revLogID := logID + "-revocations"
		revStore, err := m.storeManager.GetStore(revLogID)
		if err != nil {
			return fmt.Errorf("failed to get state store for revocations log %s: %w", revLogID, err)
		}
		if err := storacha.Recover(ctx, storacha.Config{
			SpaceDID:   logID, // revocations are stored in the same space
			LogDID:     revLogID,
			StateStore: revStore,
			GatewayURL: gatewayURL,
			Logger:     m.logger,
		}, revocationsHeadCID); err != nil {
			return fmt.Errorf("recover revocations log: %w", err)
		}
	}
